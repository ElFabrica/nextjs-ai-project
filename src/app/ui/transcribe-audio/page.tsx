...
export default function Page() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Falha ao descrever o áudio!");
      }
  ...
  return (
    <div className="flex flex-col w-full max-w-md py-24 mx-auto stretch">
      ...
      {isLoading && (
        <div className="text-center mb-4">Transcrevendo áudio...</div>
      )}
        ...
        <div className="mb-8 p-4 bg-zinc-100 dark:bg-zinc-800 rounded-lg">
          ...
          {transcript.language && (
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
              Linguagem: {transcript.language}
            </p>
          )}

          {transcript.durationInSeconds && (
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Duração: {transcript.durationInSeconds.toFixed(1)} seconds
            </p>
          )}
        </div>
      )}
        ...
        <div className="flex flex-col gap-2">
          {selectedFile && (
            <div className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-400">
              <span>Selecionado: {selectedFile.name}</span>
              <button
                type="button"
                onClick={resetForm}
